...
local: check
	make build-init
	$(eval OBS_WORKDIR := $(BUILD_TARGET)/$(ZENDGUARD_TARGET))
...
obs: check zendguard54 zendguard55 zendguard56

build-binary-target:
	make build-init
	$(eval OBS_WORKDIR := $(BUILD_TARGET)/$(ZENDGUARD_TARGET))
	cat macros/$(ZENDGUARD_TARGET) SPECS/scl-zendguard.spec > OBS/$(OBS_WORKDIR)/scl-zendguard.spec
...
zendguard54 zendguard55 zendguard56:
	ZENDGUARD_TARGET=$@ make build-binary-target
...
build-init: build-clean
	$(eval OBS_WORKDIR := $(BUILD_TARGET)/$(ZENDGUARD_TARGET))
	mkdir OBS
	osc branch $(OBS_PROJECT) $(OBS_PACKAGE) $(BUILD_TARGET) $(ZENDGUARD_TARGET) 2>/dev/null || exit 0
